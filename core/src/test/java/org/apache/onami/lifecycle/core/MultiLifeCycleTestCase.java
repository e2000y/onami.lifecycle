...
public class MultiLifeCycleTestCase
{
    ...
    {
        Module lifeCycleModule = new TestLifeCycleModule1( MultiLifeCycleObject.class );
        MultiLifeCycleObject obj = Guice.createInjector( lifeCycleModule ).getInstance( MultiLifeCycleObject.class );
        Assert.assertEquals( "s", obj.toString() );
    }
    ...
    public void testStaging()
    {
        ...
        Module moduleD =
            new TestLifeCycleStageModule1( new DefaultStager<LifecycleInterface>( LifecycleInterface.class ) );

        Injector injector = Guice.createInjector( moduleA, moduleB, moduleC, moduleD );
        ...
        Assert.assertEquals( "aaabbbc", obj.toString() );
        injector.getInstance( LifeCycleStageModule.key( LifecycleInterface.class ) ).stage();
        Assert.assertEquals( "aaabbbcs", obj.toString() );
    ...
    private static class TestLifeCycleModule1 extends LifeCycleModule
    {

        private final Class<?> clz;

        public TestLifeCycleModule1( Class<?> clz )
        {
            this.clz = clz;
        }
    ...
    private static class TestLifeCycleStageModule1 extends LifeCycleStageModule
    {

        private final Stager<?> stager;

        public TestLifeCycleStageModule1( Stager<?> stager )
        {
            this.stager = stager;
        }
